...
def readauthor_year():
    ...
    for i in range(len(author)):
        ...
        a = a.split(';',500)
        author_set.append(a)
    return author_set,year


######绘图
def author_plot(author_set,year):
    start = min(year)
    end = max(year)+1
    ...
    ax.plot(x,author_num[0],marker='o',label='1-5')
    ax.plot(x,author_num[1],marker='o',label='6-10')
    ax.plot(x,author_num[2],marker='o',label='>'+str(10))
    ...
    plt.savefig('3-1.eps', format='eps',bbox_inches='tight')
    plt.show()


if __name__ == '__main__':
    author_set,year =readauthor_year()
    author_plot(author_set,year)
